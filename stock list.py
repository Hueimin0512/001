...
if not st.session_state.df.empty:
    delete_index = st.number_input("删除第几行？（输入行号 ITEM）", min_value=1, max_value=len(st.session_state.df), step=1)
    if st.button("删除此行"):
        ...
        st.success(f"已删除第 {delete_index} 行并保存！")

# 显示表格
if not st.session_state.df.empty:
    st.markdown(f"### 📋 Stock List as at {stock_date.strftime('%Y-%m-%d')}")
