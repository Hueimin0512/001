...
if "stock_date" not in st.session_state:
    st.session_state.stock_date = datetime.date.today()

# 日期选择
stock_date = st.date_input("📅 Stock as at", value=st.session_state.stock_date)
...
# 显示表格
st.header(f"记录总览（Stock as at {stock_date.strftime('%Y-%m-%d')}）")

if not st.session_state.df.empty:
    st.markdown(st.session_state.df.to_html(index=False), unsafe_allow_html=True)

    # 删除记录功能
    delete_index = st.number_input("输入要删除的 ITEM 编号", min_value=1, step=1, value=1)
    if st.button("删除该记录"):
        ...
        st.session_state.df.to_csv(DATA_FILE, index=False)
        st.success(f"已删除 ITEM {delete_index} ！")
...
# 下载 Excel
def to_excel(df, stock_date_str):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df.to_excel(writer, index=False, sheet_name=f"Stock as at {stock_date_str}")
